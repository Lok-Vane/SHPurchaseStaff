...
import { DaoService, EPurchaseSettingService, SharedService } from 'src/app/core';

@Component({
  selector: 'app-epu-vieprice-edit',
  templateUrl: './epu-vieprice-edit.component.html',
  styleUrls: ['./epu-vieprice-edit.component.less']
})
export class EpuViePriceEditComponent implements OnInit {
  meteringCopy = '';
  ...
  ngOnInit() {
    ...
    this.dao.doPostRequest(this.SHPurchase.ViePriceDetail, postData).subscribe((res: any) => {
      ...
      if (new Date(this.data.startTime).getTime() < new Date().getTime()) {
        alert('当前超出项目允许修改时间！');
        this.router.navigate(['/epu/viepricelist', this.data.bizId]);
      }
      ...
      this.shared.Form1Copy = {
        ...
        seedGoodsTime: this.data.seedGoodsTime,    // 交货完成时间
      ...
      // 查询一级分类名称
      this.dao.doPostRequest(this.SHPurchase.FirCategoryByCode, this.shared.Form1Copy.category).subscribe((res1: any) => {
  ...
  back() {
    this.router.navigate(['epu/viepricedetail', this.data.bizId]);
  }
  ...
  doModify() {
    ...
    this.dao.doPostRequest(this.SHPurchase.ViePriceModify, postData).subscribe((res: any) => {
